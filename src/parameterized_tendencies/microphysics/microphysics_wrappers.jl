...
single quadrature pass: at saturated points (`q_lcl_eq_hat > 0`)
condensate is concentrated where it physically should be; at subsaturated
points (`q_lcl_eq_hat = 0`) only the β-blend remnant remains, which
evaporates against the local subsaturated `q_v_hat`, capturing
below-cloud rain evaporation and snow sublimation.
...
`q_lcl_hat = q_lcl_mean · (γ_l · q_lcl_eq_hat + β_l)` with
`γ_l · M_l + β_l = 1`, where `M_l = ⟨q_lcl_eq⟩` is precomputed in the
`compute_sgs_moments` pre-pass. This gives **exact mass conservation by
construction** and unifies the cloudy and clear regimes into one
